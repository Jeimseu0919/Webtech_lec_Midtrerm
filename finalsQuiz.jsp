...
<html>
    ...
    <body>
        ...
        <form method="POST" action="FinalsQuizChecker">   
            <h1>Identification</h1>            
            <sql:query dataSource="${db}" var="rs">
                SELECT description, answer FROM questions 
                WHERE term='finals' AND type='identification';
            </sql:query> 
            <ol>
            ...
            <sql:query dataSource="${db}" var="rs">
                SELECT description, answer FROM questions 
                WHERE term='finals' AND type='TrueOrFalse';
            </sql:query> 
            <table>
                <c:forEach var="questionList" items="${rs.rows}"> 
                    <tr>
            ...
            <sql:query dataSource="${db}" var="rs">
                ...
                WHERE term='finals' AND type='Multiplechoice';
            </sql:query>
            <ol>
            ...
            <h1>Matching Type</h1>     
            <p>Determine it is <strong>PHP</strong> or <strong>NODE</strong>.</p>   
            
            <sql:query dataSource="${db}" var="rs">
                SELECT description, answer FROM questions 
                WHERE term='finals' AND type='Matchingtype';
            </sql:query> 
            <table>
                <c:forEach var="questionList" items="${rs.rows}">
